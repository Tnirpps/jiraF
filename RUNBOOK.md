...
#### 5.4 Обновить маппинг исполнителей

Использовать, если:
- в Todoist-проект добавили нового участника
- у пользователя сменился Telegram username
- бот начал предлагать не того исполнителя

Шаги:
1. В Telegram выбрать правильный проект через `/set_project`
2. Отправить `/set_assignee_map`
3. Ответить на сообщение бота YAML-файлом документом
...
Ожидаемый результат:
- бот отвечает `Маппинг исполнителей обновлён`
- в логах нет ошибок `failed to insert assignee mapping`

Типовые ошибки:
- `email "... " не найден среди участников выбранного Todoist-проекта`:
  проверить выбранный проект и состав участников в Todoist
- `Не удалось импортировать YAML-маппинг`:
  проверить структуру файла и обязательные поля
...
### 7.5 Исполнитель определяется неверно

Признаки:
- в preview показывается не тот assignee
- после подтверждения задача в Todoist назначается не тому человеку

Шаги:
1. Проверить, что в чате выбран правильный Todoist-проект
2. Проверить, что для этого проекта загружен актуальный YAML через `/set_assignee_map`
3. Проверить, что нужный пользователь есть среди участников Todoist-проекта
4. Проверить логи:
   ```bash
   docker-compose logs bot | grep -iE "assignee|collaborator|mapping"
   ```

Важно:
- бот не назначает исполнителя по одному только `@упоминанию`
- итоговое решение принимает AI среди кандидатов, разрешённых YAML-маппингом
